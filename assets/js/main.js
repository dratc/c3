  ...
  /*---------- 01. On Load Function ----------*/
    ...
    $.fn.thmobilemenu = function (options) {
        ...
        return this.each(function () {
            ...
            // Toggle dropdown when clicking the parent link
    ...
    function updateNavHeight() {
        if ($('body').hasClass('header-is-sticky')) {
            var navHeight = $('.sticky-wrapper').outerHeight() || 0;
            var heightVal = navHeight + 'px';
            document.documentElement.style.setProperty('--nav-height', heightVal);
            document.body.style.setProperty('--nav-height', heightVal);
        } else {
    ...
    // Run once on load to ensure correct initial value
    ...
    // Duplicate scroll listener removed to prevent conflict
    ...
    $('.th-slider').each(function () {
        ...
        // Disable loop if not enough slides to prevent Swiper Loop Warning
        var spv = sliderDefault.slidesPerView === 'auto' ? 1 : (sliderDefault.slidesPerView || 1);
        if (sliderDefault.breakpoints) {
            for (var bp in sliderDefault.breakpoints) {
                if (sliderDefault.breakpoints[bp].slidesPerView && sliderDefault.breakpoints[bp].slidesPerView !== 'auto') {
                    spv = Math.max(spv, sliderDefault.breakpoints[bp].slidesPerView);
                }
            }
        }
        if (sliderDefault.loop && thSlider.find('.swiper-slide').length <= spv) {
            sliderDefault.loop = false;
        }
